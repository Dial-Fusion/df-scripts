...
$api_log_only=0;
$api_archive_only=0;
$skip_optimize_source=0;
$skip_optimize_archive=0;
$skip_count_source=0;
$skip_count_archive=0;
...
if (length($ARGV[0])>1)
	...
	if ($args =~ /--help/i)
		...
		print "  [--skip-optimize-source] = OPTIONAL, skips running the OPTIMIZE TABLE statements on the source tables being archived from\n";
		print "  [--skip-optimize-archive] = OPTIONAL, skips running the OPTIMIZE TABLE statements on the tables being inserted into\n";
		print "  [--skip-count-source] = OPTIONAL, skips retrieving a count of records in the source tables being archived from\n";
		print "  [--skip-count-archive] =  OPTIONAL, skips retrieving a count of records in the archive tables being inserted into\n";
		print "  [--quiet] = quiet\n";
		...
		if ($args =~ /--skip-optimize-source/i) 
			{
			$skip_optimize_source=1;
			print "\n----- SKIP OPTIMIZE SOURCE TABLES -----\n\n";
			}
		if ($args =~ /--skip-optimize-archive/i) 
			{
			$skip_optimize_archive=1;
			print "\n----- SKIP OPTIMIZE ARCHIVE TABLES -----\n\n";
			}
		if ($args =~ /--skip-count-source/i) 
			{
			$skip_count_source=1;
			print "\n----- SKIP ROW COUNT SOURCE TABLES -----\n\n";
			}
		if ($args =~ /--skip-count-archive/i) 
			{
			$skip_count_archive=1;
			print "\n----- SKIP ROW COUNT ARCHIVE TABLES -----\n\n";
			}
		if ($args =~ /--daily/i)
			{
			$daily=1;
...
if (!$T) 
	...
	if ($only_trim_archive > 0)
		...
		if ($skip_count_archive > 0)
			{$stmtA = "SELECT 'ARCHIVE COUNT SKIPPED';";}
		else
	...
	if ($daily > 0)
		...
		if ($skip_count_source > 0)
			{$stmtA = "SELECT 'SOURCE COUNT SKIPPED';";}
		else
	...
	if ($queue_log > 0)
		...
		##### queue_log
		if ($skip_count_source > 0)
			{$stmtB = "SELECT 'SOURCE COUNT SKIPPED';";}
		else
		...
		if ($skip_count_archive > 0)
			{$stmtB = "SELECT 'ARCHIVE COUNT SKIPPED';";}
		else
